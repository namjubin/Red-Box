...
class Joystick:
    def __init__(self, screen):
        ...
        self.surface_size = (self.screen_size[0], int((self.screen_size[0]/self.size[0])*self.size[1]))
        self.surface_loc = (0, (self.screen_size[1]//2)-(self.surface_size[1]//2))
    ...
    def push(self, data):
        ...
        try:
            for num, value in enumerate(self.btn_state):
                if data[num+3]:
                    if not value:
                        result[num] = True
                        self.btn_state[num] = True
                else:
                    self.btn_state[num] = False
            return result
        except:
            return result
    ...
    def start(self):
        while self.run:
            ...
            if self.data:
                if self.button == [True,True,True,True]:
                    self.run = False
                if self.joystick[-1]:
